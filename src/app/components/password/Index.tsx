      ...
      {/* <!-- Modal content --> */}
        <form onSubmit={onSubmit} className="flex flex-col w-full h-full p-6 text-center bg-white rounded-3xl" autoComplete="none" >
          ...
          <div className="flex w-full columns-2">
            ...
            <button className="border bg-gray-200 hover:bg-gray-400 text-black font-bold px-3 py-3 mb-3 rounded">
            Send
            </button>
          </div>
          <label htmlFor="name" className="mb-2 text-m text-start text-grey-900">Number</label>
          <div className="flex w-full columns-2">
            <input disabled={true} autoComplete="off" id="name" type="text" placeholder="Number"  className="border flex items-center w-full px-5 py-3 mb-3 mr-2 text-m  focus:bg-grey-400 placeholder:text-grey-700 bg-grey-200 text-dark-grey-900 rounded"/>
            <button disabled={true} className="cursor-not-allowed border bg-gray-200 hover:bg-gray-400 text-black font-bold px-3 py-3 mb-3 rounded">
            Confirm
          </button>
          </div>
          <label htmlFor="password" className="mb-2 text-m text-start text-grey-900">Password</label>
          <input disabled={true} autoComplete="new-password" id="password" type="password" placeholder="Password" className="border flex items-center w-full px-5 py-3 mb-3 mr-2 text-m  focus:bg-grey-400 placeholder:text-grey-700 bg-grey-200 text-dark-grey-900 rounded"/>
          
          <label htmlFor="repassword" className="mb-2 text-m text-start text-grey-900">Repassword</label>
          <input disabled={true} autoComplete="new-password" id="repassword" type="password" placeholder="Repassword" className=" border flex items-center w-full px-5 py-3 mb-3 mr-2 text-m  focus:bg-grey-400 placeholder:text-grey-700 bg-grey-200 text-dark-grey-900 rounded"/>

          <button disabled={true} className="cursor-not-allowed border bg-gray-200 hover:bg-gray-400 text-black font-bold py-3 px-4 mt-3 rounded mb-5" >
          Change Password
          </button>
